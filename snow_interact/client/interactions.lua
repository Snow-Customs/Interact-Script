...
AddEventHandler('snow_interact:groupsChanged', function(groups)
    myGroups = groups or {}
...
function api.addEntityInteraction(data)
    ...
    if NetworkDoesNetworkIdExist(netId) then
        if not Entity(NetworkGetEntityFromNetworkId(netId)).state.hasInteractOptions then
            TriggerServerEvent('snow_interact:setEntityHasOptions', netId)
        end
    end
...
RegisterNetEvent('snow_interact:removeEntity', function(data)
    for i = 1, #data do
        local netId = data[i]
